locals{
  ...
  igw_name  = var.name != "" ? var.name : (var.vpc_name!= null ? "${var.vpc_name}-igw" : "${local.prefix_name}-igw")
  #igw_name  = var.name != "" ? var.name : "${local.prefix_name}-igw"
  igw_id = var.provison ? aws_internet_gateway.internet_gw[0].id : ""
  igw_arn = var.provison ? aws_internet_gateway.internet_gw[0].arn : ""
  vpc_id =var.provison ? data.aws_vpc.vpc[0].id : null
  
}


resource null_resource print_names {
  count = var.provison ? 1 : 0
...
resource "aws_internet_gateway" "internet_gw" {
  ...
  count = var.provison && var._count > 0 ?  1 : 0
  vpc_id = local.vpc_id
  tags ={ 
      Name = local.igw_name, 
